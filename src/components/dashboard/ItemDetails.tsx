...
//       style={tw`${index < array.length - 1 ? 'mb-3' : ''} text-gray-800`}
//       theme={{
//         colors: {
//           background: '#fff',
//           text: '#000000',
...
const ItemDetails: React.FC = () => {
      ...
      <Portal>
        <Modal
          ...
          contentContainerStyle={[
            ...
            {
              maxHeight: Dimensions.get('window').height * 0.8, // Limit modal height to 80% of screen
              width: Math.min(Dimensions.get('window').width - 32, 500), // Responsive width
            },
          ]}>
          <ScrollView
            ...
            <Text style={tw`text-xl font-semibold mb-6 text-black`}>
              {editingItem ? 'Edit Item' : 'Add New Item'}
            </Text>
  ...
  keyboardType={type === 'numeric' ? 'numeric' : 'default'}
  onChangeText={text => setNewItem({ ...newItem, [key]: text })}
  style={tw`${index < array.length - 1 ? 'mb-3' : ''}`}
  theme={{
    colors: {
      background: '#fff',
      text: 'black',
...
/>
  ...
  <Button
    ...
    contentStyle={tw`py-3`} // padding inside the button
  >
    Cancel
  </Button>
