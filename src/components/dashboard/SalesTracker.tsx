...
const SalesTracker = () => {
  ...
  useEffect(() => {
    const fetchCustomerData = async () => {
      try {
        setIsLoading(true);
        const customerId = (await AsyncStorage.getItem('customerId')) || 'demo_schema';
  ...
  const fetchSalesData = async (customerId: string) => {
    try {
      ...
      Toast.show({
        type: 'error',
        text1: 'Error',
        text2: error.response?.status === 404 ? 'Schema or data not found' : 'Failed to load sales data',
        position: 'bottom',
      });
    }
  };
  ...
  const handleAddNew = async () => {
    const { sales_date, total_sales, gross_income, total_expenses, net_income } = newSales;

    if (!sales_date || !total_sales || !gross_income || !total_expenses || !net_income) {
      Toast.show({
        type: 'error',
        text1: 'Error',
    ...
    try {
      ...
      Toast.show({
        type: 'error',
        text1: 'Error',
        text2: error.response?.data?.error || 'Failed to add sales record',
        position: 'bottom',
      });
    } finally {
      setIsLoading(false);
  ...
  return (
    <Provider>
      <View style={tw`flex-1 bg-gray-50`}>
        ...
        />
        <View style={tw`flex-1 p-4 mt-${isMobile ? '16' : '0'}`}>
          <View style={tw`flex-row px-4 py-3 bg-white border-b border-gray-200 justify-between`}>
            <TouchableOpacity
          ...
          {isLoading && !modalVisible ? (
            ...
            <>
              ...
              <View style={tw`bg-white rounded-lg shadow overflow-hidden`}>
                <FlatList
                  data={salesData}
                  keyExtractor={(item, index) => (item.sales_date ? `${item.sales_date}-${index}` : `index-${index}`)}
            ...
            <View style={tw`bg-white rounded-lg shadow p-8 items-center`}>
              <Text style={tw`text-lg text-gray-500`}>No sales data available</Text>
            </View>
          )}
        </View>
        <Portal>
